# Bridge Observability & PII Best-Effort Checker
...
- Custom structured HTTP access logs output one parseable structured event per request with fields including: `request_id`, `method`, `path`, `status`, `latency_ms`, `app_id` (if authenticated), and `error_code` (on failure).
...
```text
Observability and PII best-effort verdict: PASS / FAIL / NOT APPLICABLE
