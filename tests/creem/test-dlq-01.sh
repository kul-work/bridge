...
##############################################################################
# DLQ-01: Webhook Forwarding Exhausts Retries → Dead-Lettered (Creem)
#
...
#   failed attempts. Complements NET-01 (retry/backoff case). This covers
#   the exhaustion case where the app is consistently unreachable.
...
#   other webhook-injection tests in the CBI suite. Driving the forwarder
#   through 3 real HTTP failures would require pointing the app callback
#   URL at an unreachable endpoint and waiting for the scheduler, which
#   is not deterministic enough for a shell test.
##############################################################################
...
echo -e "${YELLOW}========================================${NC}"
echo "DLQ-01: Webhook Forwarding Exhausts Retries → Dead-Lettered (Creem)"
echo -e "${YELLOW}========================================${NC}"
echo ""
