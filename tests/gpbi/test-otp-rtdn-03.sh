...
# 
# Purpose: Verify that a oneTimeProductNotification with notificationType: 2
#          (REFUNDED) is properly received and updates payment status.
#
# Usage: ./test-otp-rtdn-03.sh [--token "purchase_token"]
#
# Prerequisites:
...
TEST_RUN_ID="otp-rtdn-03-${TIMESTAMP}-$$"
PRODUCT_ID="$PRODUCT_ID_OTP"
PROVIDER="$PROVIDER"
REPORT_FILE="otp-rtdn-03-report.json"
PURCHASE_TOKEN=""
USER_ID="${USER_ID:-test_otp_user_03_$TEST_RUN_ID}"
...
echo -e "${YELLOW}========================================${NC}"
echo "OTP-RTDN-03: Webhook OTP Refunded (Type 2)"
echo -e "${YELLOW}========================================${NC}"
echo ""
...
if [[ -z "$PURCHASE_TOKEN" ]]; then
    ...
    PURCHASE_TOKEN="test-otp-token-03-$TEST_RUN_ID"
...
MESSAGE_ID="webhook-otp-refund-$TEST_RUN_ID"
...
echo -e "${YELLOW}[2/3] Sending ONE_TIME_PRODUCT_REFUNDED webhook...${NC}"
...
if [[ "$STATUS" == "refunded" ]]; then
    echo -e "${GREEN}✓ Success: Status is 'refunded'${NC}"
    TEST_STATUS="pass"
else
    echo -e "${RED}✗ Failure: Status is '$STATUS', expected 'refunded'${NC}"
    TEST_STATUS="fail"
fi
...
{
  "test_id": "OTP-RTDN-03",
  "test_name": "Webhook OTP Refunded (Type 2)",
