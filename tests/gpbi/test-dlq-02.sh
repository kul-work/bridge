...
#
# Test Approach:
#   We seed a dead-lettered delivery via SQL (same as DLQ-01), then:
#   1. If ADMIN_JWT is set: call POST /admin/webhooks/:id/retry and verify
#      the delivery row is reset.
#   2. If ADMIN_JWT is not set: call the reset_webhook_delivery DB function
#      directly (simulating what the admin handler does internally) and
#      verify the same reset behavior.
#   Then we verify the ADMIN-WHK-02 regression: attempting to retry an
#   already-forwarded delivery should NOT reopen it.
##############################################################################
...
echo -e "${YELLOW}========================================${NC}"
echo "DLQ-02: Dead-Lettered Webhook Recovered via Admin Retry"
echo -e "${YELLOW}========================================${NC}"
echo ""
