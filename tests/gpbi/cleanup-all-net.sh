...
# Manual check: Bridge does not track emails. Set USER_ID externally or use default.
    ...
    if [[ ! -z "$USER_ID" ]] && [[ "$USER_ID" != *"error"* ]] && [[ "$USER_ID" != *"ERROR"* ]]; then
        ...
        psql -U "$DATABASE_USER" -h "$DATABASE_HOST" -p $DATABASE_PORT -d "$DATABASE_NAME" -c "DELETE FROM pay.subscriptions WHERE external_user_id = '$USER_ID' AND purchase_token LIKE '%net-%';" 2>/dev/null
        echo -e "${GREEN}✓ Removed NET test subscription records${NC}"
        
        # Delete NET test pay.payments
        psql -U "$DATABASE_USER" -h "$DATABASE_HOST" -p $DATABASE_PORT -d "$DATABASE_NAME" -c "DELETE FROM pay.payments WHERE external_user_id = '$USER_ID' AND provider_transaction_id LIKE '%net-%';" 2>/dev/null
        echo -e "${GREEN}✓ Removed NET test payment records${NC}"
        
        # Clean up any orphan NET test records
        psql -U "$DATABASE_USER" -h "$DATABASE_HOST" -p $DATABASE_PORT -d "$DATABASE_NAME" -c "DELETE FROM pay.subscriptions WHERE purchase_token LIKE 'test-net-%';" 2>/dev/null
