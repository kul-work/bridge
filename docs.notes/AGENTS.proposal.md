...
Before changing code in `src/webhooks/*`, `src/application/verify_purchase*`, `src/db/payments.rs`, `src/db/subscriptions.rs`, `src/services/google_play/*`, or any provider callback path, follow the applicable checkers in `CHECKERS.md`:

1. **Classify and claim** (CHECKERS §1.1–1.2): PARITY → use `bridge-hiha-parity` skill first. UNKNOWN → stop, investigate.
2. **Field-level assertions** (CHECKERS §1.3): after implementation, verify tests assert affected durable fields — not just HTTP 200.
3. **Bug guardrail** (CHECKERS §1.4): every payment bug fix must leave behind an assertion that would have caught it.

Before any release → use `bridge-release-loop` skill (which invokes `bridge-release-gate` for risk classification per CHECKERS §5).

For unclassifiable or cross-cutting tasks → fill the intake template in CHECKERS §6 before implementing.
