...
- **Status:** Fixed (2026-06-26) — reconciliation now keyed by row id; forward stale-check and retry/resume rebuild are Google purchase-token keyed with no SKU fallback; regression coverage added in `src/webhooks/identity_cross_user_tests.rs`, `src/webhooks/forwarding.rs::stale_suppression_branch_tests`, and GPBI `tests/gpbi/test-whk-07.sh`.
...
So the central concept was understood. What was missed: **three side-paths silently reverted to `subscription_id`-only keying** and drop the user/purchase-token dimension. With one license tester at a time they never fired. With two real customers on the same product (`hiha_monthly`) in different lifecycle states, they corrupt entitlement — silently, no error, looks fine in logs.

This is not "the whole Google integration is wrong." It is "three side paths need the user/purchase-token dimension preserved." Scare is real, scope is small.
...
**Failure mode:** A retry meant for user A can send user B's same-SKU lifecycle state to HiHa.
...
- **Retry/resume rebuild:** needs a failed or resumed callback delivery plus two same-SKU rows. Happy-path manual sessions rarely exercise callback rebuild after delivery failure.
...
The author understood Google's identity model and encoded it in `resolve_user` and the tracing. But that knowledge was not enforced as an invariant at the repository boundary. Later side paths (`update_subscription_status`, the forward stale-check, retry/resume payload rebuild) reached for the convenient `subscription_id` key without the user/purchase-token dimension, and nothing stopped them. The practical guardrail is exactly that: lifecycle mutations and row-derived payload rebuilds for `google_play` must not accept a `subscription_id`-only key.
...
- Retry/resume payload rebuild is required for this bug, not follow-up.
