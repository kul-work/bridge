...
These scenarios are shared with the [Google Play testplan](../google/GOOGLE_PLAY_BILLING_TESTPLAN.md) and the [Bridge Admin testplan](../testing/BRIDGE_ADMIN_TESTPLAN.md). They are listed here for completeness so the Creem acceptance can verify the full lifecycle including operator intervention.
...
- ✅ **Admin Retry**: Dead-lettered webhook recovered via admin retry without duplicate DB entries (DLQ-02); see [BRIDGE_ADMIN_TESTPLAN.md](../testing/BRIDGE_ADMIN_TESTPLAN.md).
- ✅ **Admin Auth**: Admin endpoints reject mismatched `azp`, wrong issuer, and missing tokens; see [BRIDGE_ADMIN_TESTPLAN.md](../testing/BRIDGE_ADMIN_TESTPLAN.md) acceptance criteria.
