...
| **ERR-04** | **Revoked/Refunded Purchase Token** | 1. Make a purchase, verify it successfully.<br>2. Go to Play Console > Orders, **Revoke** the order.<br>3. App re-checks status (call `/api/v1/subscriptions` or similar). | - Status should show revoked/expired.<br>- Access should be revoked. | - Backend `get_subscription()` call to Google returns state indicating revocation.<br>- Either webhook `purchase.voided` sent, OR next status check shows status change.<br>- DB updated: status → `Cancelled` or `Expired`. | Revocation is immediate in Play Console; webhook or polling will sync to app. |
...
| **NOTIF-01** | **Payment Failure & Acknowledgment** | 1. Trigger Account Hold (SUB-08).<br>2. Call `GET /api/v1/subscriptions`.<br>3. Verify `payment_failure_notification=true`.<br>4. Call `POST /api/v1/subscriptions/:subscription_id/acknowledge`.<br>5. Call `GET /api/v1/subscriptions` again. | - Step 2: Response shows failure flag and message.<br>- Step 4: Success (200 OK).<br>- Step 5: `payment_failure_notification=false`. | - DB updates `subscriptions.payment_failure_notification` to `false`.<br>- In-app alert cleared. | Ensures persistent alerts can be dismissed by user. |
...
- **`GET /api/v1/subscriptions`** (authenticated): Returns current premium status and expiration.
