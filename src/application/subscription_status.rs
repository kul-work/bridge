...
pub async fn get_subscription_status_snapshot<R: SubscriptionReadRepository + ?Sized>(
    ...
    Ok(match select_subscription_for_snapshot(&subscriptions) {
        Some(sub) => SubscriptionStatusSnapshot {
            ...
            google_pending_price_change_currency: sub.google_pending_price_change_currency.clone(),
            google_pending_price_change_mode: sub.google_pending_price_change_mode.clone(),
            google_pending_price_change_state: sub.google_pending_price_change_state.clone(),
